...
**Rule resolution has a specific precedence**: `ftq_parts.csv` membership forces `MATERIAL` to `Aluminum 3003 CHK FTQ` regardless of the matched description rule (`description_rules.csv`) — check `ftq_parts.csv` first when a material output looks wrong. `bom_reader.py` handles column detection/coercion (including treating invalid/blank quantities as `0`); `nonlaser_tokens.csv` and `expected_laser_descriptions.csv` drive the missing-DXF classification prompts (non-laser vs. expected-laser-but-missing).

**Local CSV config files (`description_rules.csv`, `ftq_parts.csv`, `nonlaser_tokens.csv`, `expected_laser_descriptions.csv`, `laser_materials.csv`) are created automatically if missing** — don't add defensive existence checks around them elsewhere; `rule_store.py` owns that.
